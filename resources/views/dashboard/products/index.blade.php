<x-front-layout>
    ...
    <x-slot:extra_nav>
        ...
        <div class="nav-item mx-2">
            <a href="{{ route('dashboard.products.create') }}" class="btn btn-icon text-success m-0">
        ...
        <div class="col-md-12" style="padding: 0 2px;">
            <div class="card">
                <div class="card-body table-container p-0">
                    <table id="products-table" class="table table-striped table-bordered table-hover sticky" style="width:100%; height: calc(100vh - 155px);">
                        <thead>
                            <tr>
                                <th></th>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                let table = $('#products-table').DataTable({
                    ...
                    columns: [
                        { data: 'edit', name: 'edit', orderable: false, class: 'text-center', searchable: false, render: function(data, type, row) {
                            ...
                            let link = `<a href="{{ route('dashboard.products.edit', ':product') }}" class="btn btn-icon btn-icon text-primary edit_row"  data-id=":product"><i class="fa-solid fa-edit"></i></a>`.replace(':product', data);
                            return link ;
                            @else
                            return '';
                            @endcan
                    ...
                    footerCallback: function(row, data, start, end, display) {
                        ...
                        //     }, 0);
                        // $('#products-table_filter').addClass('d-none');
                    }
                });
                $('#products-table_filter').addClass('d-none');
                ...
                function deleteRow(id) {
                    $.ajax({
                        url: '{{ route("dashboard.products.destroy", ":id") }}'.replace(':id', id),
                        method: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}",
                        },
