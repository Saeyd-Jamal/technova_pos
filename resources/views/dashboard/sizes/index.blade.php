<x-front-layout>
    ...
    <x-slot:extra_nav>
        ...
        <div class="nav-item mx-2">
            <a href="{{ route('dashboard.sizes.create') }}" class="btn btn-icon text-success m-0">
        ...
        <div class="col-md-12" style="padding: 0 2px;">
            <div class="card">
                <div class="card-body table-container p-0">
                    <table id="sizes-table" class="table table-striped table-bordered table-hover sticky" style="width:100%; height: calc(100vh - 155px);">
                        <thead>
                            <tr>
                                <th></th>
    ...
    @push('scripts')
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                let table = $('#sizes-table').DataTable({
                    processing: true,
                    serverSide: true,
                    responsive: true,
                    ...
                    columns: [
                        { data: 'edit', name: 'edit', orderable: false, class: 'text-center', searchable: false, render: function(data, type, row) {
                            ...
                            let link = `<a href="{{ route('dashboard.sizes.edit', ':size') }}" class="btn btn-icon btn-icon text-primary edit_row"  data-id=":size"><i class="fa-solid fa-edit"></i></a>`.replace(':size', data);
                            return link ;
                            @else
                            return '';
                            @endcan
                    ...
                    drawCallback: function(settings) {
                        ...
                        $('#sizes-table tbody tr').each(function() {
                    ...
                    footerCallback: function(row, data, start, end, display) {
                        ...
                        //     }, 0);
                        // $('#sizes-table_filter').addClass('d-none');
                    }
                });
                $('#sizes-table_filter').addClass('d-none');
                ...
                function deleteRow(id) {
                    $.ajax({
                        url: '{{ route("dashboard.sizes.destroy", ":id") }}'.replace(':id', id),
                        method: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}",
                        },
