<x-front-layout>
    ...
    <x-slot:extra_nav>
        ...
        <div class="nav-item mx-2">
            <a href="{{ route('dashboard.financialdiaries.create') }}" class="btn btn-icon text-success m-0">
        ...
        <div class="col-md-12" style="padding: 0 2px;">
            <div class="card">
                <div class="card-body table-container p-0">
                    <table id="financialdiaries-table" class="table table-striped table-bordered table-hover sticky" style="width:100%; height: calc(100vh - 155px);">
                        <thead>
                            <tr>
                                <th></th>
    ...
    @push('scripts')
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                let table = $('#financialdiaries-table').DataTable({
                    processing: true,
                    serverSide: true,
                    responsive: true,
                    ...
                    columns: [
                        { data: 'edit', name: 'edit', orderable: false, class: 'text-center', searchable: false, render: function(data, type, row) {
                            @can('update','App\Models\FinancialDiary')
                            let link = `<a href="{{ route('dashboard.financialdiaries.edit', ':financialdiariary') }}" class="btn btn-icon btn-icon text-primary edit_row"  data-id=":financialdiariary"><i class="fa-solid fa-edit"></i></a>`.replace(':financialdiariary', data);
                            return link ;
                            @else
                            return '';
                            @endcan
                        ...
                        { data: 'delete', name: 'delete', orderable: false, class: 'text-center', searchable: false, render: function (data, type, row) {
                            @can('delete','App\\Models\FinancialDiary')
                            return `
                                <button
                    ...
                    drawCallback: function(settings) {
                        ...
                        $('#financialdiaries-table tbody tr').each(function() {
                    ...
                    footerCallback: function(row, data, start, end, display) {
                        ...
                        //     }, 0);
                        // $('#financialdiaries-table_filter').addClass('d-none');
                    }
                });
                $('#financialdiaries-table_filter').addClass('d-none');
                ...
                function deleteRow(id) {
                    $.ajax({
                        url: '{{ route("dashboard.financialdiaries.destroy", ":id") }}'.replace(':id', id),
                        method: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}",
                        },
