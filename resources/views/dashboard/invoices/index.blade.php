        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                const table = $('#invoices-table').DataTable({
                    ...
                    columns: [
                        { data: 'edit', name: 'edit', orderable: false, class: 'text-center', searchable: false, render: function(data, type, row) {
                            @can('update','App\Models\Invoice')
                            let link = `<a href="{{ route('dashboard.invoices.edit', ':invoice') }}" class="btn btn-icon btn-icon text-primary edit_row"  data-id=":invoice"><i class="fa-solid fa-edit"></i></a>`.replace(':invoice', data);
                            return link ;
                            @else
                            return '';
                            @endcan
                    ...
                    footerCallback: function(row, data, start, end, display) {
                        ...
                        //     }, 0);
                        // $('#invoices-table_filter').addClass('d-none');
                    }
                });
                $('#invoices-table_filter').addClass('d-none');
                ...
                function deleteRow(id) {
                    $.ajax({
                        url: '{{ route("dashboard.invoices.destroy", ":id") }}'.replace(':id', id),
                        method: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}",
                        },
