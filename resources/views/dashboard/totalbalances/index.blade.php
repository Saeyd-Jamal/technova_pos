<x-front-layout>
    ...
    @endpush
    <x-slot:extra_nav>
        @can('create', 'App\\Models\TotalBalance')
        <div class="nav-item mx-2">
            <a href="{{ route('dashboard.totalbalances.create') }}" class="btn btn-icon text-success m-0">
        ...
        <div class="col-md-12" style="padding: 0 2px;">
            <div class="card">
                <div class="card-body table-container p-0">
                    <table id="totalbalances-table" class="table table-striped table-bordered table-hover sticky" style="width:100%; height: calc(100vh - 155px);">
                        <thead>
                            <tr>
                                <th></th>
    ...
    @push('scripts')
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                let table = $('#totalbalances-table').DataTable({
                    processing: true,
                    serverSide: true,
                    responsive: true,
                    ...
                    ajax: {
                        url: '{{ route("dashboard.totalbalances.index") }}',
                    ...
                    columns: [
                        { data: 'edit', name: 'edit', orderable: false, class: 'text-center', searchable: false, render: function(data, type, row) {
                            @can('update','App\Models\TotalBalance')
                            let link = `<a href="{{ route('dashboard.totalbalances.edit', ':category') }}" class="btn btn-icon btn-icon text-primary edit_row"  data-id=":category"><i class="fa-solid fa-edit"></i></a>`.replace(':category', data);
                            return link ;
                            @else
                            return '';
                            @endcan
                        ...
                        { data: 'delete', name: 'delete', orderable: false, class: 'text-center', searchable: false, render: function (data, type, row) {
                            @can('delete','App\\Models\TotalBalance')
                            return `
                                <button
                    ...
                    drawCallback: function(settings) {
                        ...
                        $('#totalbalances-table tbody tr').each(function() {
                    ...
                    footerCallback: function(row, data, start, end, display) {
                        ...
                        //     }, 0);
                        // $('#totalbalances-table_filter').addClass('d-none');
                    }
                });
                $('#totalbalances-table_filter').addClass('d-none');
                ...
                function deleteRow(id) {
                    $.ajax({
                        url: '{{ route("dashboard.totalbalances.destroy", ":id") }}'.replace(':id', id),
                        method: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}",
                        },
